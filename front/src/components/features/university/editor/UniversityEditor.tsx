...
import {
  SaveButton,
  CancelButton,
} from "@/components/features/university/buttons/EditButtons";
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            type="number"
            value={
              department.majors[0]?.admissionSchedules[0]?.admissionInfos[0]
                ?.enrollment || 0
            }
            onChange={(e) =>
              handleInfoChange("enrollment", parseInt(e.target.value, 10))
            }
            className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
          />
        </div>
        <div>
          ...
          {department.majors[0]?.admissionSchedules[0]?.testTypes.map(
            (testType) => (
                ...
                {testType.subjects.map((subject) => (
                  <div key={subject.id} className="mt-2 flex gap-2">
                    <input
                      ...
                      value={subject.name}
                      onChange={(e) =>
                        handleSubjectNameChange(subject.id, e.target.value)
                      }
                      className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                    />
                    <input
