...
describe('DepartmentInfo', () => {
  describe('表示モード', () => {
    it('大学名、学部名、学科名、日程、募集人数が正しく表示されること', () => {
      const props = createTestDepartmentInfo();
      render(<DepartmentInfo {...props} />);
  ...
  describe('編集モード', () => {
    it('編集可能な入力フィールドが表示されること', () => {
      const props = { ...createTestDepartmentInfo(), isEditing: true };
      render(<DepartmentInfo {...props} />);
      ...
      expect(screen.getByDisplayValue('前')).toBeInTheDocument();
    ...
    it('日程を変更できること', () => {
      ...
      const select = screen.getByDisplayValue('前');
      fireEvent.change(select, { target: { value: '後' } });

      expect(props.onInfoChange).toHaveBeenCalledWith('schedule', '後期');
    });
