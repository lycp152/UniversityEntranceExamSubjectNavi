...
export const useScoreTable = (scores: SubjectScores) => {
  return useMemo(() => {
    try {
      ...
      // 科目名のソート
      const sortedSubjects = Object.keys(calculatedScores).sort((a, b) => a.localeCompare(b, 'ja'));

      // 合計値の計算
      const totals = Object.values(calculatedScores).reduce<ScoreTableTotals>(
        (acc, { [TEST_TYPES.COMMON]: common, [TEST_TYPES.INDIVIDUAL]: individual, total }) => ({
          [TEST_TYPES.COMMON]: acc[TEST_TYPES.COMMON] + common.score,
          [TEST_TYPES.INDIVIDUAL]: acc[TEST_TYPES.INDIVIDUAL] + individual.score,
