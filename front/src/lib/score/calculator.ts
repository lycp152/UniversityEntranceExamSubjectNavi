...
import { ScoreCalculationError, ScoreErrorCodes, createErrorMessage } from '../errors/score';
...
export class ScoreCalculator {
  ...
  constructor(options: ScoreCalculatorOptions = {}) {
    this.roundToDecimal = options.roundToDecimal ?? SCORE_CONSTRAINTS.DEFAULT_DECIMAL_PLACES;
  }
  ...
  protected calculateBaseScore(score: BaseScore, field?: string): ScoreMetrics {
    try {
      ...
      throw new ScoreCalculationError(
        ...
        field,
        { value: score.value, maxValue: score.maxValue }
      );
    }
  }
  ...
  calculateSubjectScore(subject: BaseSubjectScore): SubjectScoreDetail {
    try {
      const commonTest = this.calculateBaseScore(subject[TEST_TYPES.COMMON], TEST_TYPES.COMMON);
      ...
      const totalMaxScore =
        subject[TEST_TYPES.COMMON].maxValue + subject[TEST_TYPES.INDIVIDUAL].maxValue;
   ...
   * 全科目の合計スコアを計算
   */
  calculateTotalScores(subjects: SubjectScores): Record<string, SubjectScoreDetail> {
    try {
   ...
   * テスト種別ごとの合計を計算
   */
  calculateTestTypeTotal(subjects: SubjectScores, testType: keyof typeof TEST_TYPES): number {
