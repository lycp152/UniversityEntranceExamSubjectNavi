...
const SUBJECT_TO_CATEGORY_MAP: Record<string, SubjectCategory> = {
  ...
  [SUBJECTS.JAPANESE]: SUBJECT_CATEGORIES.ENGLISH,
...
export class ScoreAggregator {
  aggregateByCategory(scores: Score[]): CategoryScore[] {
    ...
    return Array.from(categoryMap.entries()).map(
      ([category, categoryScores]) => ({
        category,
        common: this.calculateMetrics(
          categoryScores.filter((s) => s.type === "共通")
        ),
        individual: this.calculateMetrics(
          categoryScores.filter((s) => s.type === "二次")
        ),
  ...
  private calculateMetrics(scores: Score[]): ScoreMetrics {
    const totalValue = scores.reduce(
      (sum, score) => sum + score.value * score.weight,
      0
    );
    const maxValue = scores.reduce(
      (sum, score) => sum + score.maxValue * score.weight,
      0
    );
