...
export const validateScore = (score: Score): ValidationResult<Score> => {
  ...
  if (score.value < SCORE_CONSTRAINTS.MIN_VALUE || score.value > score.maxValue) {
    errors.push({
  ...
  if (score.weight < SCORE_CONSTRAINTS.MIN_WEIGHT || score.weight > SCORE_CONSTRAINTS.MAX_WEIGHT) {
    errors.push({
